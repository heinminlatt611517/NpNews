...
import com.heinminlatt.npnews.views.viewHolders.RelatedMoviesViewHolder
import com.heinminlatt.shared.adapter.BaseRecyclerAdapter
import com.heinminlatt.shared.view.viewHolders.BaseViewHolder

class RelatedMovieAdapter(delegate : RelatedMoviesItemDelegate) : BaseRecyclerAdapter<BaseViewHolder<Int>, Int>() {
    var mDelegate  = delegate
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<Int> {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.list_items_related_movies, parent, false)
        return RelatedMoviesViewHolder(mDelegate,view)
    }
}